...
import { LayoutDashboard, Users, Home, Calendar, BarChart3, Shield, Bell, MessageSquare, Settings, LogOut, CheckCircle, XCircle, Clock, TrendingUp, Eye, KeyRound, Heart, Star } from "lucide-react";
...
const DashboardSidebar = ({ active, role }: { active: string; role: string }) => {
  const { user } = useAuth();
  const agentLocked = role === "agent" && agentNeedsLicenseActivation(user);
  ...
  const links: Record<string, { icon: any; label: string; href: string }[]> = {
    ...
    agent: [
      ...
      { icon: Star, label: "Reviews", href: "/agent/reviews" },
  ...
  return (
    <aside className="w-64 section-dark min-h-screen p-6 flex flex-col fixed left-0 top-0">
      ...
      <nav className="flex-1 space-y-1">
        {(agentLocked ? links.agent.slice(0, 1) : links[role] || links.admin).map((link) => (
          <Link
            key={link.label}
            to={link.href}
