...
const AgentReviews = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["agent-reviews", user?._id],
    queryFn: () => tourService.list({ status: "completed" }),
  });

  const tours = (data?.data || []).filter(
    (tour) => tour.feedback && tour.agentId?._id === user?._id
  );

  const averageRating =
    tours.length > 0
      ? tours.reduce((sum, tour) => sum + (tour.feedback?.agentRating || 0), 0) / tours.length
      : 0;

  const ratingDistribution = [5, 4, 3, 2, 1].map((rating) => ({
    rating,
    count: tours.filter((t) => t.feedback?.agentRating === rating).length,
  }));

  return (
    <div className="min-h-screen bg-muted flex">
      <DashboardSidebar active="Reviews" role="agent" />
      <main className="flex-1 ml-64 p-8">
        <div className="mb-8">
          <h1 className="text-3xl font-heading font-bold text-foreground mb-2">Your Reviews</h1>
          <p className="text-sm text-muted-foreground">View feedback from clients about your service</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
            <div className="text-2xl font-bold text-blue-600">
              {tours.filter((t) => t.feedback?.overallExperience === "excellent").length}
            </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-6">
          <h2 className="text-lg font-heading font-bold text-foreground mb-4">Rating Distribution</h2>
              ...
              <div key={rating} className="flex items-center gap-4">
                ...
                <div className="flex-1 bg-muted rounded-full h-2 relative overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${tours.length > 0 ? (count / tours.length) * 100 : 0}%`,
                    }}
                  />
                </div>
